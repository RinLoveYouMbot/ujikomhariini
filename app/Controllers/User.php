...
class User extends BaseController
{
    ...
    private $rules = [
        'nama'     => ['rules' => 'required', 'errors' => ['required' => 'Nama Wajib Di Isi']],
        'username' => ['rules' => 'required|alpha_numeric|is_unique[tb_users.username,id,{id}]', 'errors' => ['required' => 'Username Wajib Di Isi', 'is_unique' => 'username telah terdaftar','alpha_numeric'=>'kolom hanya boleh diisi oleh huruf dan nomor']],
        'email'    => ['rules' => 'required|valid_email|is_unique[tb_users.email,id,{id}]', 'errors' => ['required' => 'email Wajib Di Isi', 'is_unique' => 'email telah terdaftar','valid_email'=>'hanya boleh diisi oleh email @gmail']],
        'role'     => ['rules' => 'required', 'errors' => ['required' => 'role Wajib Di Isi']],
    ];
    ...
    {
        if ($this->request->isAJAX()) {
            $this->rules['password'] = ['rules' => 'required', 'errors' => ['required' => 'Password Wajib Di Isi']];
        ...
        // return $this->response->setJSON(array("OK"));
    }
    ...
    public function ubah_profil()
    {
        ...
        if ($this->request->isAJAX()) {
            // $this->rules['password'] = ['rules' => 'min_length[8]', 'errors' => ['min_length' => 'Panjang password minimal 8 kaarakter']];
            if (!$this->validate($this->rules)) {
